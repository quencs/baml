...
# Create .env for sage-backend from example only if user hasn't created one
SAGE_ENV_DIR="typescript/apps/sage-backend"
SAGE_ENV_FILE="$SAGE_ENV_DIR/.env"
SAGE_ENV_EXAMPLE="$SAGE_ENV_DIR/.env.example"
if [ -d "$SAGE_ENV_DIR" ] && [ -f "$SAGE_ENV_EXAMPLE" ]; then
    if [ ! -f "$SAGE_ENV_FILE" ]; then
        echo -e "${YELLOW}📦 Creating ${SAGE_ENV_FILE} from example...${NC}"
        cp "$SAGE_ENV_EXAMPLE" "$SAGE_ENV_FILE"
        echo -e "${GREEN}✅ Created ${SAGE_ENV_FILE}${NC}"
    else
        echo -e "${GREEN}✅ ${SAGE_ENV_FILE} already exists — leaving it alone${NC}"
    fi
fi
...
# removing Ruby check for now as we don't have any Ruby code currently
