...
Timeouts can be configured on:
- **Leaf clients** (OpenAI, Anthropic, etc.): All timeout types except `total_timeout_ms`
- **Composite clients** (fallback, round-robin): All timeout types including `total_timeout_ms`
...
<ParamField path="total_timeout_ms" type="int">
  Maximum time for the entire strategy execution, including all client attempts.

  **Default:** No timeout (infinite)

  **Only valid on composite clients** (fallback, round-robin).
  ...
  client<llm> MyFallback {
    ...
    options {
      ...
      total_timeout_ms 120000     // 2 minutes total
    }
  }
  ```

  <Warning>
  If you try to use `total_timeout_ms` on a non-composite client, BAML will raise a validation error.
  </Warning>
</ParamField>
...
The `total_timeout_ms` of 120000ms applies to both clients combined.
...
2. **Composite-only:** `total_timeout_ms` can only be used on fallback/round-robin clients
