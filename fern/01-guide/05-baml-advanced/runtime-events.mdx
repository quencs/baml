...
You can track changes to emitted variables from the client code. When you generate
client code from your BAML code, we produce event listener structs.
...
class MakePostsEventCollector:
    ...
    def on_var_progress_percent(self, handler: Callable[[VarEvent[int]], None]) -> None:
