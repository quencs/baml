...
Add timeouts to any client by specifying timeout values in the `http_client` block within `options`:
...
**Effective timeouts:**
- `FastClient`: `min(5000, ∞)` connect, `min(∞, 20000)` request = 5s connect, 20s request
- `SlowClient`: `min(5000, ∞)` connect, `min(∞, 60000)` request = 5s connect, 60s request
