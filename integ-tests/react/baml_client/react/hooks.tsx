...
/**
 * A specialized hook for the StreamTest BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 *
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useStreamTest({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useStreamTest({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useStreamTest(props: HookInput<'StreamTest', { stream: false }>): HookOutput<'StreamTest', { stream: false }>
export function useStreamTest(props?: HookInput<'StreamTest', { stream?: true }>): HookOutput<'StreamTest', { stream: true }>
export function useStreamTest(
  props: HookInput<'StreamTest', { stream?: boolean }> = {},
): HookOutput<'StreamTest', { stream: true }> | HookOutput<'StreamTest', { stream: false }> {
  let action: ServerAction = Actions.StreamTest;
  if (isStreamingProps(props)) {
    action = StreamingActions.StreamTest;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'StreamTest', { stream: false }>)
  }
}
/**
