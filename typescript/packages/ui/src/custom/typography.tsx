...
export type TextTypographyProps = TypographyProps & VariantProps<typeof textVariants>

export const H1 = twx.h1.transientProps(['variant'])<TypographyProps & TwcComponentProps<'h1'>>(({ variant }) =>
  cn(typographyVariants({ variant }), 'text-balance font-semibold text-4xl tracking-tight'),
)

export const H2 = twx.h2.transientProps(['variant'])<TypographyProps & TwcComponentProps<'h2'>>(({ variant }) =>
  cn(typographyVariants({ variant }), 'text-balance font-semibold text-3xl tracking-tight first:mt-0'),
)

export const H3 = twx.h3.transientProps(['variant'])<TypographyProps & TwcComponentProps<'h3'>>(({ variant }) =>
  cn(typographyVariants({ variant }), 'text-balance font-semibold text-2xl tracking-tight'),
)
export const H4 = twx.h4.transientProps(['variant'])<TypographyProps & TwcComponentProps<'h4'>>(({ variant }) =>
  cn(typographyVariants({ variant }), 'text-balance font-semibold text-2xl leading-8'),
)
export const P = twx.p.transientProps(['variant', 'size', 'textFlow', 'spacing'])<
  TextTypographyProps & TwcComponentProps<'p'>
>(({ size, variant, textFlow }) => cn(typographyVariants({ variant }), textVariants({ size, textFlow })))
...
>(({ size, variant }) => cn('prose', proseTypographyVariants({ variant }), textVariants({ size })))

export const Text = twx.span.transientProps(['size', 'variant', 'textFlow', 'spacing'])<
  TextTypographyProps & TwcComponentProps<'div'>
>(({ size, variant, textFlow, spacing }) =>
  cn(typographyVariants({ variant }), textVariants({ size, spacing, textFlow })),
)

export type TextLoadingProps = TwcComponentProps<'div'> & VariantProps<typeof textLoadingVariants>

export const TextLoading = twx(Skeleton).transientProps(['size'])<TextLoadingProps>(({ size }) =>
  textLoadingVariants({ size }),
)
