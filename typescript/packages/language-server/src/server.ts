...
import type { SymbolLocation } from '@gloo-ai/baml-schema-wasm-node';
...
export function startServer(options?: LSOptions): void {
    ...
    try {
      switch (params.type) {
        ...
        case 'diagnostic':
          ...
          connection
            .sendRequest('runtime_diagnostics', { errors, warnings })
            .catch((e) => {
  ...
  const getConfig = async () => {
    try {
      const configResponse =
        await connection.workspace.getConfiguration('baml');
    ...
    if (bamlConfig.config?.cliPath) {
      ...
      try {
        ...
        if (version) {
          ...
          throw new Error(
            `Error parsing baml-cli version from output: ${stdout}`,
          );
        }
      } catch (error) {
        console.error(error);
  ...
  connection.onDidChangeWatchedFiles((params) => {
    ...
    if (hasChanges) {
      ...
      bamlProjectManager
        .reload_project_files(URI.parse(params.changes[0]?.uri ?? ''))
        .catch((e) => {
  ...
  documents.onDidChangeContent(async (change: { document: TextDocument }) => {
    ...
    await bamlProjectManager.upsert_file(
  ...
  documents.onDidSave(async (change: { document: TextDocument }) => {
    ...
    console.log(
      'saving uri' + documentUri.toString() + '   ' + documentUri.fsPath,
    );
          ...
          () => {
            connection.sendNotification('baml/message', {
              ...
              message:
                ...
                bamlConfig.cliVersion +
                ')',
            });
          },
        );
      } else {
        await bamlProjectManager
          .getProjectById(documentUri)
          ?.runGeneratorsWithDebounce({
  ...
  function restartTSServer() {
    if (bamlConfig.config?.restartTSServerOnSave) {
      connection
        .sendRequest('executeCommand', 'typescript.restartTsServer')
        .catch((e) => {
        ...
        if (proj) {
          for (const func of proj.list_functions()) {
            if (URI.file(func.span.file_path).toString() === document.uri) {
              const range = Range.create(
                document.positionAt(func.span.start),
                document.positionAt(func.span.end),
              );
          ...
          for (const testcase of proj.list_testcases()) {
            ...
            if (URI.file(testcase.span.file_path).toString() === document.uri) {
              ...
              testcase.parent_functions.forEach((parentFunction: any) => {
                const command: Command = {
                  title:
                    testcase.parent_functions.length > 1
                      ? `▶ Run for ${parentFunction.name}`
                      : '▶ Run Test',
  ...
  connection.onRequest('textDocument/rename', (params: RenameParams) => {
    ...
    if (project.runtime().is_valid_function(symbol)) {
      throw new ResponseError(
        ...
        `Function renaming is not yet supported: '${symbol}'`,
      );
    }
