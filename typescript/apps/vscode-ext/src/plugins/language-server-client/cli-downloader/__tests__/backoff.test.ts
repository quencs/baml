...
describe('BackoffManager', () => {
  ...
  describe('shouldAttemptDownload', () => {
    it('should allow download when no previous failures', () => {
      const result = backoffManager.shouldAttemptDownload('1.0.0', mockOutputChannel);
    ...
    it('should enforce backoff after failure', () => {
      ...
      const result1 = backoffManager.shouldAttemptDownload('1.0.0', mockOutputChannel);
      expect(result1).toBe(false);
      
      // Should allow after backoff period
      vi.setSystemTime(now + 11 * 60 * 1000); // 11 minutes later
      const result2 = backoffManager.shouldAttemptDownload('1.0.0', mockOutputChannel);
    ...
    it('should increase backoff time with multiple failures', () => {
      ...
      expect(backoffManager.shouldAttemptDownload('1.0.0', mockOutputChannel)).toBe(false);
      
      vi.setSystemTime(now + 11 * 60 * 1000 + 21 * 60 * 1000); // 21 minutes after second failure
      expect(backoffManager.shouldAttemptDownload('1.0.0', mockOutputChannel)).toBe(true);
    });
  });
  ...
  describe('failure count reset', () => {
    it('should reset failure count after max failures', () => {
      ...
      expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(
        expect.stringContaining('Resetting count but maintaining backoff timestamp')
      );
    });
  });
});