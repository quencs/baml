...
import * as crypto from 'node:crypto'; // Added for hash calculation
import fs from 'node:fs'; // Use synchronous fs methods where appropriate for path checking
...
import {
  ...
  ProgressLocation,
  window,
} from 'vscode'; // Import ExtensionContext, window, and OutputChannel
...
const downloadBackoffState = new Map<
  string,
  { failureCount: number; lastAttemptTimestamp: number }
>();
...
function _getCliCompressedFileExtension(platform: string): string {
  ...
  switch (getReleasePlatform(platform)) {
    ...
    default:
      console.warn(
        `Unsupported platform ${getReleasePlatform(platform)} for compression extension, defaulting to zip.`,
      );
 ...
 */
function cliBinaryArtifactName({
  architecture,
  platform,
  version,
}: CliVersion): string {
 ...
 * Internal helper.
 */
async function _ensureInstallPathExists(
  context: ExtensionContext,
): Promise<string> {
  const installPath = getInstallPath(context);
  try {
...
async function _extractFile(
  ...
  bamlOutputChannel.appendLine(
    `Extracting ${sourcePath} to: ${binaryFilePath}`,
  );
  ...
  if (extension === 'tar.gz') {
    ...
    bamlOutputChannel.appendLine(
      `Tarball extracted and renamed to: ${binaryFilePath}`,
    );
    ...
    if (entry) {
      bamlOutputChannel.appendLine(
        `Found entry ${zipEntryName} in zip. Extracting to ${binaryFilePath}`,
      );
      ...
      bamlOutputChannel.appendLine(
        `Extracted ${zipEntryName} to ${binaryFilePath}`,
      );
    } else {
      bamlOutputChannel.appendLine(
...
export async function downloadCli(
  ...
  bamlOutputChannel.appendLine(
    ` Temp Download Path: ${compressedFileTempPath}`,
  );
  ...
  try {
    ...
    bamlOutputChannel.appendLine(
      `Downloading binary to ${compressedFileTempPath}...`,
    );
    ...
    bamlOutputChannel.appendLine(
      `Binary download complete: ${compressedFileTempPath}`,
    );
    ...
    try {
      ...
      bamlOutputChannel.appendLine(
        `ERROR: Failed to download or parse checksum file: ${checksumError.message}`,
      );
      if (
        axios.isAxiosError(checksumError) &&
        checksumError.response?.status === 404
      ) {
        bamlOutputChannel.appendLine(
          `WARN: Checksum file not found at ${checksumUrl}. Cannot verify integrity.`,
        );
    ...
    bamlOutputChannel.appendLine(
      `Calculating SHA256 hash for ${compressedFileTempPath}...`,
    );
    ...
    const now = Date.now();
    const state = downloadBackoffState.get(cliVersion.version) ?? {
      failureCount: 0,
    ...
    if (downloadSucceeded) {
      try {
        bamlOutputChannel.appendLine(
          `Cleaning up temporary file: ${compressedFileTempPath}`,
        );
  ...
  if (targetTriple) {
    ...
    if (platform === 'linux' && targetTriple.endsWith('-gnu')) {
      ...
      if (fs.existsSync(muslBundledPath)) {
        console.log(
          'Found bundled CLI at MUSL fallback path:',
          muslBundledPath,
        );
...
export async function resolveCliPath(
  ...
  if (os.platform() !== 'win32' && bundledCliActualPath) {
    ...
    const targetCachePathForBundled = downloadedCliPath(
      context,
      cliVersionMetaForBundled,
    );
    ...
    try {
      ...
      try {
        ...
        await fs.promises.copyFile(
          ...
          targetCachePathForBundled,
        );
        bamlOutputChannel.appendLine(
          `Successfully copied bundled CLI to cache: ${targetCachePathForBundled}`,
        );
  ...
  if (backoffInfo) {
    ...
    const backoffDelay = Math.min(
      INITIAL_BACKOFF_DELAY_MS * 2 ** (failureCount - 1),
