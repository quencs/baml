...
mod tests {
    ...
    async fn test_primitive_types() -> Result<()> {
        ...
        assert_eq!(result.string_field, "Hello, BAML!", "Expected string_field to be 'Hello, BAML!'");
        assert_eq!(result.int_field, 42, "Expected int_field to be 42");
        assert!((result.float_field - 3.14159).abs() < 0.001, "Expected float_field to be approximately 3.14159");
        assert_eq!(result.bool_field, true, "Expected bool_field to be true");
        assert!(result.null_field.is_none(), "Expected null_field to be None");
    ...
    async fn test_primitive_arrays() -> Result<()> {
        ...
        // Verify array values
        assert_eq!(result.string_array, vec!["hello", "world", "baml"], "Expected string_array values");
        assert_eq!(result.int_array, vec![1, 2, 3, 4, 5], "Expected int_array values");
        assert_eq!(result.float_array.len(), 4, "Expected float_array length 4");
        assert!((result.float_array[0] - 1.1).abs() < 0.001, "Expected first float to be approximately 1.1");
        assert_eq!(result.bool_array, vec![true, false, true, false], "Expected bool_array values");
    ...
    async fn test_primitive_maps() -> Result<()> {
        ...
        // Verify map contents
        assert_eq!(result.string_map.len(), 2, "Expected string_map to have 2 entries");
        assert!(result.string_map.contains_key("key1"), "Expected string_map to contain 'key1'");
        assert!(result.string_map.contains_key("key2"), "Expected string_map to contain 'key2'");
        
        assert_eq!(result.int_map.len(), 3, "Expected int_map to have 3 entries");
        assert!(result.int_map.contains_key("one"), "Expected int_map to contain 'one'");
        assert!(result.int_map.contains_key("two"), "Expected int_map to contain 'two'");
        assert!(result.int_map.contains_key("three"), "Expected int_map to contain 'three'");
        
        assert_eq!(result.float_map.len(), 2, "Expected float_map to have 2 entries");
        assert!(result.float_map.contains_key("pi"), "Expected float_map to contain 'pi'");
        assert!(result.float_map.contains_key("e"), "Expected float_map to contain 'e'");
        
        assert_eq!(result.bool_map.len(), 2, "Expected bool_map to have 2 entries");
        assert!(result.bool_map.contains_key("isTrue"), "Expected bool_map to contain 'isTrue'");
        assert!(result.bool_map.contains_key("isFalse"), "Expected bool_map to contain 'isFalse'");
    ...
    async fn test_mixed_primitives() -> Result<()> {
        ...
        assert!(result.measurements.len() > 0, "Expected measurements to have content");
    ...
    async fn test_top_level_float() -> Result<()> {
        ...
        assert!((result - 3.14159).abs() < 0.001, "Expected approximately 3.14159");
