...
/// XML-specific output format renderer
#[derive(Debug)]
...
impl std::fmt::Display for OutputFormatXml {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let content = self
            .text
            .render_xml(RenderOptions::default())
            .map_err(|_e| std::fmt::Error {})?;
...
// XML-specific output format object implementation
impl minijinja::value::Object for OutputFormatXml {
    fn call(
        self: &std::sync::Arc<Self>,
        _state: &minijinja::State<'_, '_>,
        args: &[minijinja::value::Value],
        ...
        let Ok(_) = kwargs.assert_all_used() else {
            return Err(Error::new(
                ErrorKind::TooManyArguments,
                "output_format_xml() got an unexpected keyword argument (only 'prefix', 'always_host_enums', 'enum_value_prefix', and 'or_splitter' are allowed)",
            ));
        };

        // Use XML rendering instead of JSON rendering
        let content = self.text.render_xml(RenderOptions::new(
            prefix,
            or_splitter,
            enum_value_prefix,
            always_hoist_enums,
    ...
    fn call_method(
        ...
        Err(minijinja::Error::new(
            ErrorKind::UnknownMethod,
            format!("output_format_xml has no callable attribute '{}'", name),
        ))
    }
