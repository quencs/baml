...
fn main() {
    ...
    if args.len() < 2 || args.len() > 3 {
        ...
        eprintln!(
            "Example: {} engine/baml-lib/ast/examples/complex_headers_test.baml",
            args[0]
        );
        eprintln!(
            "Example: {} --fancy engine/baml-lib/ast/examples/complex_headers_test.baml",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    match parse(root_path, &source) {
        Ok((ast, _diagnostics)) => {
            // Generate container-based Mermaid diagram with optional styling
            let mermaid_diagram = BamlVisDiagramGenerator::generate_with_styling(&ast, use_fancy);

            println!(
                "Generated BamlVis Mermaid Diagram for '{}' (styling: {}):",
                baml_file_path,
            ...
            println!("4. View the rendered containerized header diagram!");
        }
        Err(err) => {
