...
func initializeBaml() error {
	...
	cPath := C.CString(bamlSharedLibraryPath)
	...
	if handle == nil {
		...
		dlopenErr := fmt.Errorf("dlopen error for %s: %s", bamlSharedLibraryPath, dlErrStr)
		if strings.Contains(dlErrStr, "mach-o, but wrong architecture") || strings.Contains(dlErrStr, "wrong ELF class") {
			dlopenErr = fmt.Errorf("%w (architecture mismatch: host is %s/%s)", dlopenErr, runtime.GOOS, runtime.GOARCH)
	...
	func() {
		...
		lib.registerFn("create_baml_runtime")
		lib.registerFn("destroy_baml_runtime")
		lib.registerFn("invoke_runtime_cli")
	...
	if symbolLookupErr != nil {
		C.dlclose(bamlLibHandle)
		bamlLibHandle = nil
		err := fmt.Errorf("%w: failed to register required functions: %w", ErrLoadLibrary, symbolLookupErr)
		return err
	}
...
func (l *library) registerFn(fnName string) {
	fnPtr := getFromLibraryFn(l.handle, fnName)
	switch fnName {
	case "version":
...
func getFromLibraryFn(handle unsafe.Pointer, fnName string) unsafe.Pointer {
	...
	C.dlerror()
	fnPtr := C.dlsym(handle, cFnName)
	errStr := C.GoString(C.dlerror())

	if fnPtr == nil {
		errMsg := fmt.Sprintf("panic: critical error looking up symbol '%s' in BAML library %s.",
			fnName, bamlSharedLibraryPath)
		if errStr != "" {
			errMsg += fmt.Sprintf("\n       dlerror(): %s", errStr)
		} else {
			errMsg += "\n       dlerror() reported no error, but symbol not found."
		}
