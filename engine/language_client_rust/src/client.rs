use crate::{
    BamlContext, BamlResult, BamlError, FunctionResult, StreamState,
...
use tokio::sync::{oneshot, mpsc as async_mpsc};
...
impl CallbackManager {
    ...
    fn handle_stream_event(&self, id: u32, is_final: bool, success: bool, data: String) {
        if let Some(sender) = self.pending_streams.lock().unwrap().get(&id) {
            let _ = sender.send(StreamEvent { is_final, data, success });
...
impl BamlClient {
    ...
    pub fn from_directory<P: AsRef<Path>>(path: P, env_vars: HashMap<String, String>) -> BamlResult<Self> {
        ...
        fn read_baml_files(dir: &Path, files: &mut HashMap<String, String>, base_path: &Path) -> BamlResult<()> {
            let entries = fs::read_dir(dir)
                .map_err(|e| BamlError::Configuration(format!("Failed to read directory {:?}: {}", dir, e)))?;
                
            for entry in entries {
                let entry = entry.map_err(|e| BamlError::Configuration(format!("Failed to read directory entry: {}", e)))?;
                let path = entry.path();
                ...
                if path.is_dir() {
                    ...
                    let content = fs::read_to_string(&path)
                        .map_err(|e| BamlError::Configuration(format!("Failed to read file {:?}: {}", path, e)))?;
                    let relative_path = path.strip_prefix(base_path)
                        .map_err(|e| BamlError::Configuration(format!("Failed to get relative path for {:?}: {}", path, e)))?;
    ...
    pub fn from_file_content(
        ...
        let src_files_json = serde_json::to_string(files)
            .map_err(|e| BamlError::invalid_argument(format!("Failed to serialize files: {}", e)))?;
        let env_vars_json = serde_json::to_string(&env_vars)
            .map_err(|e| BamlError::invalid_argument(format!("Failed to serialize env_vars: {}", e)))?;
    ...
    pub async fn call_function_raw(&self, function_name: &str, context: BamlContext) -> BamlResult<FunctionResult> {
        // Serialize the arguments to JSON for the C FFI
        let encoded_args = serde_json::to_string(&context.args)
            .map_err(|e| BamlError::invalid_argument(format!("Failed to serialize arguments: {}", e)))?;
        ...
        let callback_result = callback_receiver.await
            .map_err(|_| BamlError::Runtime(anyhow::anyhow!("Callback channel closed")))?;
        
        if callback_result.success {
            // Parse the JSON response into a BamlValue
            let baml_value: BamlValue = serde_json::from_str(&callback_result.data)
                .map_err(|e| BamlError::deserialization(format!("Failed to parse result: {}", e)))?;
            
            Ok(FunctionResult::new(baml_value, call_id.to_string()))
        } else {
            Err(BamlError::Runtime(anyhow::anyhow!("Function call failed: {}", callback_result.data)))
        }
    }
    ...
    {
        let stream = self.call_function_stream_raw(function_name, context).await?;
        Ok(stream.map(|result| {
            match result {
                Ok(stream_state) => {
                    match stream_state {
                        StreamState::Partial(value) => {
                            T::from_baml_value(value).map(StreamState::Partial)
                        }
                        StreamState::Final(value) => {
                            T::from_baml_value(value).map(StreamState::Final)
                        }
                    }
                }
                Err(e) => Err(e),
            ...
            Poll::Ready(Some(event)) => {
                if event.success {
                    ...
                    match serde_json::from_str::<BamlValue>(&event.data) {
                        ...
                        Err(e) => Poll::Ready(Some(Err(BamlError::deserialization(
                            format!("Failed to parse stream event: {}", e)
                        )))),
                    }
                } else {
                    Poll::Ready(Some(Err(BamlError::Runtime(
                        anyhow::anyhow!("Stream event failed: {}", event.data)
                    ))))
                }
            }
...
mod tests {
    ...
    fn test_builder() {
        ...
        assert_eq!(builder.env_vars.get("TEST_VAR"), Some(&"test_value".to_string()));
    }
}